...
def main():
    parser = argparse.ArgumentParser(description="SMS Bomber for Turkiye")
    parser.add_argument("-seçimigirin", type=int, help="Select an option (1 or 2)")
    parser.add_argument("-numara", type=str, help="Enter the phone number")

    args = parser.parse_args()

    if args.seçimigirin == 1:
        # Your existing code for option 1
        pass
    elif args.seçimigirin == 2:
        # Your existing code for option 2
        pass
    else:
        print(Fore.LIGHTRED_EX + "Invalid option. Please choose 1 or 2.")
        return

    if args.numara:
        print(Fore.LIGHTYELLOW_EX + f"Phone number entered: {args.numara}")
        # Continue with your code using the provided phone number
    else:
        print(Fore.LIGHTRED_EX + "Phone number is required. Please provide a valid phone number.")
